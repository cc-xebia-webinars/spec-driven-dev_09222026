...
- [ ] T016 Implement `check_receipt` in `src/expense_validator/rules.py` (FR-006)
- [ ] T017 Treat a blank or whitespace-only receipt reference as missing
- [ ] T018 [P] Add `tests/test_receipts.py` covering the three acceptance scenarios
...
- [ ] T019 [P] Add `tests/test_currency.py` asserting conversion happens before the cap comparison
- [ ] T020 [P] Add `tests/test_approval_tiers.py` including the boundary case at exactly 500 USD
- [ ] T021 Confirm an unsupported currency exits 2 rather than guessing a rate (FR-005)

## Phase 6: Polish & Cross-Cutting Concerns

- [ ] T022 [P] Add `tests/test_cli.py` covering all three exit codes and both output formats
- [ ] T023 [P] Write `specs/001-expense-policy-validator/quickstart.md`
- [ ] T024 Confirm every `FR-###` appears in at least one test name (constitution II)
...
## Phase 7: Converge - Policy Change 2026-04-02

*Appended by `/speckit-converge` after the spec was updated for the receipt threshold
and CHF change. These are the gaps between the updated spec and the current code.*

- [ ] T025 Update `receipt_threshold_usd` in `policy/policy.json` from `"75.00"` to `"100.00"` (FR-006)
- [ ] T026 Add `"CHF"` to `supported_currencies` and add CHF monthly rates under `exchange_rates_to_usd` in `policy/policy.json` (FR-004)
- [ ] T027 [P] Update the tests that assert the old 75 USD threshold: `test_fr006_threshold_comes_from_the_policy_file` and `test_fr006_missing_receipt_above_threshold_is_a_blocking_violation` in `tests/test_receipts.py`, and `test_fr001_every_policy_value_is_read_from_the_file` in `tests/test_approval_tiers.py`
- [ ] T028 [P] Add a CHF conversion case to `tests/test_money.py`
- [ ] T029 Change `test_fr009_unsupported_currency_exits_two` in `tests/test_cli.py` to use a currency that is still unsupported, since CHF now converts

**Note**: No task touches `src/`. The change is entirely data and tests, which is
constitution principle I doing its job - and is the single clearest thing converge
shows you.
